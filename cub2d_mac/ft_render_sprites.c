...
/*   Updated: 2020/07/27 10:53:18 by atemfack         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_render_col(t_cub3d *cub3d, t_render render, float dst,
																	int height)
{
	if (dst < cub3d->ray.zbuffer[/*cub3d->map.r[Y] - */render.start[Y]/* - 1*/])
	{
		while (render.start[X] < render.end[X])
		{
			if ((render.color = cub3d->txt[SPRITE][(render.start[X]++ << T) /
							height][(render.texture_start << T) / height]))
...
void		ft_render_sprites(t_cub3d *cub3d)
{
	...
	ft_sort_sprites(cub3d, cub3d->nbr_sprite);
	i = 0;
	while (i < cub3d->nbr_sprite)
	{
		if ((gamma = atan2f(-((cub3d->sprite[i].x << T) + (TILE_2) -
				cub3d->player.x), ((cub3d->sprite[i].y << T) + (TILE_2) -
				cub3d->player.y))) < 0)
			gamma += PI_360;
		ysprite = cub3d->player.alpha + FOV_2 - gamma;
		if (gamma > PI_270 && cub3d->player.alpha < PI_90)
			ysprite = cub3d->player.alpha + FOV_2 - gamma + PI_360;
		if (cub3d->player.alpha > PI_270 && gamma < PI_90)
			ysprite = cub3d->player.alpha + FOV_2 - gamma - PI_360;
